import { saveInstance, saveAllInstances, deleteInstance } from '../actions/modelActions';
...
export class BaseModel<P>  {
    ...
    static getStoreKey = (id: string, instance: any): string => `${instance.resource}${id}`
    ...
    $saveAll(): BaseModel<P>[] {
        ...
        R.map(x => {
            return instanceObject[this.getStoreKey(x.id)] = { props: x, resource: this.resource };
        }, this.props as any)
    ...
    static list(state = store.getState()) {
        return R.values(state.model.filter((x: { resource: string, props: any }) => x.resource === this.resource ? x.props : '').map((ins: { props: any }) => ins.props).toJS())
    }
